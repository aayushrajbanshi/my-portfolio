...
export default function Lanyard() {
  ...
  return (
    <div className="relative mx-auto flex h-[360px] w-[min(78vw,230px)] justify-center overflow-visible sm:h-[500px] sm:w-[270px]" aria-hidden="true">
      <motion.div
        style={{ transformOrigin: '50% 0' }}
        animate={still ? undefined : { rotate: [3.5, -3.5, 3.5] }}
        transition={{ duration: 5.5, repeat: Infinity, ease: 'easeInOut' }}
        drag="x" dragConstraints={{ left: -20, right: 20 }} dragElastic={0.4} dragSnapToOrigin
        className="flex w-full max-w-[230px] cursor-grab flex-col items-center active:cursor-grabbing"
      >
        <div className="-mt-40 flex h-56 w-7 shrink-0 items-center justify-center overflow-hidden border-x border-white/10 bg-[#080a14] sm:-mt-48 sm:h-64 sm:w-8">
        ...
        <div className="w-full max-w-52 rounded-2xl border border-white/15 p-4 shadow-[0_24px_60px_-16px_rgba(91,108,255,0.7)] sm:max-w-56"
          ...
          <div className="grid aspect-square place-items-center overflow-hidden rounded-xl bg-gradient-to-br from-[var(--violet)] via-[var(--indigo)] to-[var(--blue)]">
            {profile.photo
          ...
          <p className="mt-3 truncate text-base font-bold leading-tight">{profile.name}</p>
          <p className="truncate text-[11px] text-cyan-300">{profile.role}</p>
